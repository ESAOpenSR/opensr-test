...
class DistanceMetric(ABC):
    ...
    def compute_image(self) -> torch.Tensor:
        return Metric(
            value=self._compute_image(self.x, self.y),
    ...
    def compute_pixel(self) -> torch.Tensor:
        return Metric(
            value=self._compute_pixel(self.x, self.y),
            description=self.name,
        )
...
class CROSSMTF(DistanceMetric):
    ...
    def __init__(
        ...
        super().__init__(x=x, y=y, method=method, patch_size=patch_size, name="CROSSMTF")
...
def get_distance(
    ...
    if method == "kl":
        ...
        distance_fn = CROSSMTF(x=x, y=y, method=agg_method, patch_size=patch_size, scale=scale)
    elif method == "lpips":
        x_rgb = x[rgb_bands, :, :]
        y_rgb = y[rgb_bands, :, :]
        distance_fn = LPIPS(x_rgb, y_rgb, method=agg_method, patch_size=patch_size, device=device)
    else:
